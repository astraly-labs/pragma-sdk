...
async def test_client_oracle_mixin_spot(pragma_client: PragmaClient):
    ...
    invocations = await pragma_client.publish_many([spot_entry_1, spot_entry_2], auto_estimate= True)
    ...
    try:
        invocations = await pragma_client.publish_many([spot_entry_future], l1_resource_bounds = ResourceBounds(max_price_per_unit=500 * 10**9,
                                           max_amount=10**7))
...
async def test_client_oracle_mixin_future(pragma_client: PragmaClient):
    ...
    invocations = await pragma_client.publish_many([future_entry_1, future_entry_2], auto_estimate= True)
    await invocations[len(invocations) - 1].wait_for_acceptance()
    # Check entries
    ...
    try:
        await pragma_client.publish_many([future_entry_future],l1_resource_bounds = ResourceBounds(max_price_per_unit=500 * 10**9,
                                           max_amount=10**7))
    except TransactionRevertedError as err:
        # err_msg = "Execution was reverted; failure reason: [0x54696d657374616d7020697320696e2074686520667574757265]"
        err_msg = "Unknown Starknet error"
