...
class MEXCFetcher(FetcherInterfaceT):
    ...
    async def fetch_pair(
        self, pair: Pair, session: ClientSession, usdt_price: float = 1, configuration_decimals: Optional[int] = None   
        ...
        async with session.get(url) as resp:
            ...
            if resp.status == 400:
                return await self.operate_usdt_hop(pair, session, configuration_decimals)
            return self._construct(pair=pair, result=result, usdt_price=usdt_price, configuration_decimals=configuration_decimals)
    ...
    async def fetch(
        ...
        for pair in self.pairs:
            entries.append(
                asyncio.ensure_future(self.fetch_pair(pair, session, usdt_price, configuration_decimals))
            )
    ...
    async def operate_usdt_hop(
        ...
        return self._construct(pair=pair, result=pair2_usdt, hop_result=pair1_usdt, configuration_decimals=configuration_decimals)
