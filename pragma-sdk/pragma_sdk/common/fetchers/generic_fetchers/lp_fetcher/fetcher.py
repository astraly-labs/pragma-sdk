...
from pragma_sdk.common.fetchers.generic_fetchers.lp_fetcher.lp_contract import (LpContract, Reserves)
...
class LPFetcher(FetcherInterfaceT): 
    ...
    def __init__(
        ...
        for address in pairs:
            self.lp_contracts[address] = LpContract(account=self.client.account, lp_address=address,)
    ...
    async def fetch_pair(
        ...
        reserves = self.get_median_reserves()
        if isinstance(reserves, PublisherFetchError):
            return reserves
        
        total_supply = self.get_median_total_supply()
    ...
    async def get_median_reserves(self, lp_contract: LpContract) -> Reserves | PublisherFetchError:
        """
        ...
        if len(history_reserves) < MINIMUM_DATA_POINTS:
            return PublisherFetchError("Can't compute Lp Price - not enough history for the Pool reserves.")
    ...
    async def get_median_total_supply(self, lp_contract: LpContract) -> int | PublisherFetchError: 
        """
        ...
        history_total_supply = self.redis_manager.get_latest_n_total_supply(
            network=self.network,
            pool_address=lp_contract.address.address,
            n=MINIMUM_DATA_POINTS,
        )

        if len(history_total_supply) < MINIMUM_DATA_POINTS:
            return PublisherFetchError("Can't compute Lp Price - not enough history for the Pool total supply.")
    
        total_supply = median(history_total_supply)
        return total_supply
        
        
    async def compute_lp_price(self, token_0: Contract, token_1: Contract, reserves: Reserves, total_supply: int,) -> int:
        """
        Computes the LP price based on reserves and total supply.
        Takes into consideration the decimals of the fetched prices.
        """

        (token_0_price, token_0_decimals) = await self.get_token_price_and_decimals(token_0)
        (token_1_price, token_1_decimals) = await self.get_token_price_and_decimals(token_1)
